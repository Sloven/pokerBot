...
namespace Classifier
{
	...
	public class MainForm : System.Windows.Forms.Form
	{
			...
			if ( openFileDialog.ShowDialog( ) == DialogResult.OK )
			{
				...
				try
				{
					...
					while ( ( samples < 200 ) && ( ( str = reader.ReadLine( ) ) != null ) )
					{
						...
						// parse tokens
                        //tempData[samples, 0] = float.Parse(strs[0], CultureInfo.InvariantCulture);
                        //tempData[samples, 1] = float.Parse(strs[1], CultureInfo.InvariantCulture);
		...
		void SearchSolution( )
		{
			...
			// create perceptron
			ActivationNetwork	network = new ActivationNetwork( new ThresholdFunction( ), variables, classesCount );
			...
			try
			{
				...
				for ( int i = 0; i < classesCount; i++ )
				{
                    ...
                    {
                        // weight 0
                        AddListSubitem(item, "Weight "+j);
                        AddListSubitem(item, layer.Neurons[i].Weights[j].ToString("F6"));
                        // weight 1
                ...
                double[,] errors = new double[errorsList.Count, variables];
