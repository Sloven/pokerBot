...
namespace Classifier
{
    public class MyTeacher : ISupervisedLearning
    {
        ...
        /// 
        public bool RunEpoch(double[][] input, double[][] output)
        {
            double error = -1;
            ...
            while (error != 0.0) //teach first layer
            {
                ...
                {
                    error += RunEpochForFirstLayer(input[i], output[i]);
                }
            }

            // set second layer 
            ActivationNeuron resolver = network.Layers[1].Neurons[0] as ActivationNeuron;
            for (int i = 1; i <= resolver.Weights.Length; i++)
                resolver.Weights[i] = i;

            return true;
        }


        public double RunEpochForFirstLayer(double[] input, double[] output)
        {
            // compute output of network
            double[] networkOutput = network.Compute(input);
