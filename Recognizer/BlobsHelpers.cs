...
namespace Recognizer
{
    ...
    public class BlobsHelpers
    {
        public static List<BitVector> ExtractBlobs(Bitmap filteredImage, List<Blob> blobs)
        {
            ...
            foreach (var blob in blobs)
            {
                ...
                bool isTableCard = (sconst > 1.4 && sconst < 1.5d);
                bool isPlayerCard = (sconst > 0.65 && sconst < 0.9);

                if (isTableCard || isPlayerCard)
                {
                    ...
                    var unrecognizedVector = GetBitVector(bmp);
                    CardSet.Add(new BitVector()
                    {
                        ...
                        Owner = isTableCard ? CardOwner.Table : isPlayerCard ? CardOwner.Player : CardOwner.Opponent
                    });
                }
            }
            return CardSet;
        ...
        public static byte[] GetBitVector(Bitmap cropedImage)
        {
