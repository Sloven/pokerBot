...
namespace Classifier
{
	...
	public class MainForm : System.Windows.Forms.Form
	{
		...
		private void loadButton_Click( object sender, System.EventArgs e )
		{
            ...
            //{
				...
				try
				{
					...
					// open selected file
					reader = File.OpenText(@"D:\myProjects\pokerBot\AForge\Samples\Neuro\Simple\Custom perceptron\Data Samples\card - Copy - Copy.csv");
					...
					while ( ( samples < 50 ) && ( ( str = reader.ReadLine( ) ) != null ) )
					{
                        ...
                        tempClasses[samples] = double.Parse(strs[variables], CultureInfo.InvariantCulture);
				...
				if ( showChart )
				{
                    chart.RangeX = new Range(float.MinValue, float.MaxValue);
                    ShowTrainingData( );
				}
        ...
        void SearchSolution()
        {
            ...
            if (net != null)
            {
                double[] testData = new double[variables];
        ...
        private void testNet(ActivationNetwork net, double[] input)
        {
            var res = net.Compute(input);
        }

        // Worker thread
        private ActivationNetwork Learning()
        {
            ...
            // create perceptron
            //ActivationNetwork	network = new ActivationNetwork( new ThresholdFunction( ), variables,new[]{samples,1});
            ActivationNetwork network = new ActivationNetwork(new BipolarSigmoidFunction(0.09f), variables, samples);
            //ActivationNetwork network = new ActivationNetwork(new AFunction(), variables, new[] { samples, 1 });
            //((ActivationLayer)network.Layers[0]).SetActivationFunction(new ThresholdFunction());
            //((ActivationLayer)network.Layers[1]).SetActivationFunction(new SigmoidFunction(2));

            //ActivationNeuron	neuron = network.Layers[0].Neurons[samples] as ActivationNeuron;
            ...
            MyBackPropagationLearning teacher = new MyBackPropagationLearning(network);
            ...
            try
            {
                ...
                double[][] thisLayerWeights = new double[samples][];
                // loop
                while (!needToStop)
                {
                    if (weightsFile != null)
                    {
                        ...
                        {
                            for (int j = 0; j < variables; j++)
                                thisLayerWeights[i][j] = network.Layers[0].Neurons[i].Weights[j];
                                //weightsFile.Write(network.Layers[0].Neurons[i].Weights[j] + ",");

                            //weightsFile.WriteLine( network.Layers[0].Neurons[i].Threshold );
                        }
                    }
                ...
                for (int i = 0; i < samples; i++)
                {
                    for (int j = 0; j < variables; j++)
                    {
                        ...
                        AddListSubitem(item, network.Layers[0].Neurons[i].Weights[j].ToString("F6"));
                    }
                    item = AddListItem(weightsList, "Threshold");
                    //AddListSubitem(item, network.Layers[0].Neurons[i].Threshold.ToString("F6"));
                }
